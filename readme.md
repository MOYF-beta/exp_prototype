...
- **启动脚本极简化**：`launch.py` 只做三件事 — 转储本次运行的实际参数（`run_param.yaml`）→ 记录当前 git 状态（`git_status/`）→ 调用 trainer 并将 stdout/stderr 重定向到日志文件。训练过程的日志输出由 trainer 自身负责
  - **⚠️ 必须使用 `subprocess.run` 调用训练入口**，不要直接 `import` 后调用 trainer 方法。只有通过子进程运行，才能正确捕获 stdout/stderr 并重定向到日志文件，同时也避免子进程的异常污染主进程。需要同时把subprocess的stdout和stderr都捕获到，并且输出到控制台同时记录到文件
